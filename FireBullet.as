package jungle
{
	 ...
	 */
	public class FireBullet extends Box2DPhysicsObject
	{
		/**
		...
		public function FireBullet(name:String, params:Object = null)
		{
			...
			onExplode = new Signal(FireBullet, Box2DPhysicsObject);
			
			var choose:Number = Math.random();
			
	/*		mc = new MovieClip(GetAssets.getAtlas().getTextures("bird_"), 8);
			
			if (choose > 0.33)
			{
				mc = new MovieClip(GetAssets.getAtlas().getTextures("orangebird_"), 8);
			}
			if (choose > 0.66)
			{
				mc = new MovieClip(GetAssets.getAtlas().getTextures("parrot_"), 8);
			}
			_velocity = new b2Vec2(-1 * (2 + 1 * Math.random()), 0);
		...
		//	   contact.get
			   ...
			   trace("fixture A contact",contact.GetFixtureB().GetBody().GetUserData().name);
			   explode(); 
		   }
		}
